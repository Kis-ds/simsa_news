...
import time
from dynamic_filters_custom import DynamicFilters
...
df_plot = df[['date_string', '중분류', '그룹명', '업체명', 'title', 'total_score', 'link', 'summary']].copy()
df_plot.columns = ['날짜', '분야', '계열명', '기업명', '기사제목', '총점', '링크', '본문요약']
...
dynamic_filters = DynamicFilters(df_plot, filters=['날짜', '분야', '계열명', '기업명', '총점'])
with st.sidebar:
...
with data_space:
    ...
    st.write(
        f'현재 관리 대상인 전체 391개의 기업 대상으로 수집된 기사 중 유의미하다고 판단된 기사(7일 기준)는 총 {df.shape[0]}건 이며, 금일 수집된 주요 기사는 총 {df[df["date_string"] == TODAY_VAR].shape[0]}건 입니다.')
    st.write(
        f'최근 주요 기사가 많이 발간된 기업은 **{plot_tree_df["업체명"][0]}**이며, {TODAY_VAR}일 기준 총 **{plot_tree_df["title"][0]}**건이 수집되었습니다.')
    ''
    ''
    ...
    with company_chart:
        ...
        ''
        st.write(
            f"{TODAY_VAR} 기준 {check_line.loc[0, '대분류']}은 {check_line.loc[0, 'total_score']}건으로 금일 전체 기사 중 {check_line.loc[0, 'per']}%로 제일 많습니다.")

        # multi line chart
        # st.markdown('<h3 style="text-align:center">   </h3>', unsafe_allow_html=True)
        clist = line_chart_df['대분류'].unique().tolist()

        dfs = {cate: line_chart_df[line_chart_df["대분류"] == cate].groupby(['date_string']).count()[
            'total_score'].reset_index() for cate in
               clist}
    ...
    with subject_chart:
        ...
        df_pivot = df.groupby('date_string').apply(lambda x: pd.Series({
            '산업_및_사업_지표': (x['a_score'] >= standard_value).sum(),  # '산업'
    ...
    dynamic_filters.display_df(height=1500, hide_index=True,
                               column_config={
                                   "링크": st.column_config.LinkColumn("URL", display_text="바로가기", width="small"),
